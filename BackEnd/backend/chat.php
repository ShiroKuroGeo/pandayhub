...
class chat
{
    ...
    private function getAllMessageQuery(){
        return "SELECT c.*, sendui.profile as sendPic, sendui.firstname, sendui.lastname, recui.profile as recPic, recui.firstname AS ref, recui.lastname AS rel FROM `chats` AS c INNER JOIN `users` AS sendui INNER JOIN `users` AS recui ON c.sender = sendui.userId AND c.reciever = recui.userId WHERE (c.`sender` = ? OR c.`reciever` = ?) AND (c.`sender` = ? OR c.`reciever` = ?) ORDER BY c.created ASC";
    }
